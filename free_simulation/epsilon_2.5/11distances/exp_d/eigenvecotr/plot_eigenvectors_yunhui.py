...
"""
color=['green','green','red','red','green','red','green','red','blue','red','green']
...
for i in range(4):
    ...
    plt.subplot(4,1,i+1)
    plt.bar(range(0,11),value,color=colors)
    plt.xticks(np.arange(0.5,11.5,1.0),label,rotation=75)
    plt.title('tICA eigenvector $\phi_%d$'%(i+1))
plt.tight_layout()
#plt.show()
plt.savefig('plot_eigenvectors_yunhui.pdf')
