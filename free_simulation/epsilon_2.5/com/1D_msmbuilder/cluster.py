...
#distance = []
#for i in range(20):
#    d=np.load('../distance_%d.npy'%i)
#    distance.append(d)
#distance=np.array(distance)
    
#tica_transformed=np.load('distance.npy')
#assignments=KCenters(n_clusters=25).fit_predict(tica_transformed)
#assignments=KCenters(n_clusters=100).fit(tica_transformed)
...
#sys.exit()
nstates=100
ass=np.load('clustered_matrix_%d.npy'%nstates)
lagtimes = np.array([1,10,100,150,200,300,400,500,600,1000,1500,2000,2500,3000])
msmts = []
...
plt.yscale('log')
plt.savefig('implied_timescales_%d.pdf'%nstates)
plt.show()
